...
import io.squark.ask.frameworkprovider.exception.AskMultipleResourceException;
...
public class ResteasyFrameworkProvider implements FrameworkProvider {
    ...
    public void provide(AskConfiguration configuration) throws AskException {
        ...
        BeanManager beanManager = (BeanManager) AskContext.getObject(BeanManager.class.getName());
        if (beanManager != null) {
        ...
        if (beanManager == null) {
            ...
            AskContext.registerObject(BeanManager.class.getName(), beanManager);
        }
        ...
        if (jaxRsCDIExtension == null) {
            throw new AskException("Failed to get JaxRsCDIExtension bean");
        }
        List<String> applications;
        if ((applications = jaxRsCDIExtension.getApplications()) != null && applications.size() > 0) {
            if (applications.size() > 1) {
                throw new AskMultipleResourceException(
