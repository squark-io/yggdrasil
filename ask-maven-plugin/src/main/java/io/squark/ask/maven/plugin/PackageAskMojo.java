package io.squark.ask.maven.plugin;
...
import io.squark.ask.core.api.model.ProviderConfiguration;
import io.squark.ask.core.api.util.Scopes;
import io.squark.ask.maven.provider.api.AskDependencyMavenUtil;
 ...
 * Copyright 2016
 */
@Mojo(name = "package-ask", defaultPhase = LifecyclePhase.PACKAGE, requiresProject = true, requiresDependencyCollection = ResolutionScope.COMPILE_PLUS_RUNTIME)
public class PackageAskMojo extends AbstractMojo {

    @Parameter(defaultValue = "${plugin}", readonly = true)
    private PluginDescriptor pluginDescriptor;
    @Parameter(property = "ask.classesDir", defaultValue = "classes")
    private String classesDir;
    @Parameter(property = "ask.configFile", defaultValue = "classes/META-INF/ask.json")
    private String configFile;
    @Parameter(property = "ask.dependencyResolutionProvider", defaultValue = "maven")
    ...
    @Parameter(property = "ask.manifest")
    private Map<String, Object> manifest;

    @Parameter(property = "ask.mainClass")
    private String mainClass;

    @Parameter(property = "ask.exclusions")
    private List<String> exclusions;

    @Parameter(property = "ask.includeLogger", defaultValue = "true")
    private boolean includeLogger;

    @Parameter(property = "ask.providerConfigurations")
    private HashSet<ProviderConfiguration> providerConfigurations;

    @Parameter(property = "ask.loadTransitiveProvidedDependencies", defaultValue = "false")
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try (JarOutputStream targetJarOutputStream = createTargetJar(manifest)) {
            ...
            logDuplicates();
            mavenProjectHelper.attachArtifact(project, project.getArtifact().getType(), "ask",
    ...
    private String getTargetJarName() {
        return project.getArtifactId() + "-" + project.getVersion() + "-ask." + project.getPackaging();
    }
    ...
    private Manifest generateAndReturnManifest() throws MojoExecutionException {
        ...
        manifest.getMainAttributes().put(Attributes.Name.MAIN_CLASS, Constants.ASK_BOOTSTRAP_CLASS_NAME);
        manifest.getMainAttributes().put(new Attributes.Name("Ask-Version"), pluginDescriptor.getVersion());
        return manifest;
    }
    ...
    private File buildConfigurationFile() throws MojoExecutionException {
        ...
        AskDependency askDependency = getProjectProvidedDependencies();
        askConfiguration.setDependencies(askDependency.getChildDependencies());
        askConfiguration.setAskVersion(pluginDescriptor.getVersion());
        askConfiguration.setLoadTransitiveProvidedDependencies(loadTransitiveProvidedDependencies);

        String classesJar = Constants.LIB_PATH + project.getArtifactId() + "-" + project.getVersion() + "-classes.jar";
        askConfiguration.setClassesJar(classesJar);

        if (StringUtils.isNotEmpty(mainClass)) {
            askConfiguration.setMainClass(mainClass);
        }

        validateProviderConfigurations();
        askConfiguration.setProviderConfigurations(providerConfigurations);

        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        String json = gson.toJson(askConfiguration);
    ...
    private void addSelfDependencies(JarOutputStream targetJarOutputStream) throws MojoFailureException, MojoExecutionException {
        ...
        addDependency(getCoreDependency(), targetJarOutputStream, true, true, Constants.ASK_RUNTIME_LIB_PATH, null);
        addDependency(getDependencyResolutionProvider(), targetJarOutputStream, true, true, Constants.ASK_RUNTIME_LIB_PATH,
                null);
        String fallbackName = Constants.ASK_LOGGING_FALLBACK_ARTIFACT_ID + ".jar";
        addDependency(getLoggingFallback(), targetJarOutputStream, true, false, Constants.ASK_RUNTIME_OPTIONAL_LIB_PATH,
                fallbackName);
    ...
    private org.apache.maven.model.Dependency getBootstrapDependency() {
        String askVersion = pluginDescriptor.getVersion();
        org.apache.maven.model.Dependency coreDependency = new org.apache.maven.model.Dependency();
        coreDependency.setGroupId(Constants.ASK_BOOTSTRAP_GROUP_ID);
        coreDependency.setArtifactId(Constants.ASK_BOOTSTRAP_ARTIFACT_ID);
        coreDependency.setVersion(askVersion);
    ...
    private org.apache.maven.model.Dependency getClassLoaderDependency() {
        String askVersion = pluginDescriptor.getVersion();
        org.apache.maven.model.Dependency coreDependency = new org.apache.maven.model.Dependency();
        coreDependency.setGroupId(Constants.ASK_GROUP_ID);
    ...
    private org.apache.maven.model.Dependency getCoreDependency() {
        ...
        org.apache.maven.model.Dependency coreDependency = new org.apache.maven.model.Dependency();
        coreDependency.setGroupId(Constants.ASK_GROUP_ID);
        coreDependency.setArtifactId(Constants.ASK_CORE_ARTIFACT_ID);
        coreDependency.setVersion(askVersion);
        coreDependency.setScope(Scopes.COMPILE);
        return coreDependency;
    }
    ...
    private org.apache.maven.model.Dependency getApiDependency() {
        ...
        apiDependency.setGroupId(Constants.ASK_GROUP_ID);
        apiDependency.setArtifactId(Constants.ASK_API_ARTIFACT_ID);
        apiDependency.setVersion(askVersion);
    ...
    private org.apache.maven.model.Dependency getDependencyResolutionProvider() throws MojoFailureException {
        ...
        switch (dependencyResolutionProviderString) {
            case "maven":
                groupId = Constants.ASK_MAVEN_PROVIDER_GROUP_ID;
                artifactId = Constants.ASK_MAVEN_PROVIDER_ARTIFACT_ID;
    ...
    private org.apache.maven.model.Dependency getLoggingFallback() {
        ...
        String askVersion = pluginDescriptor.getVersion();
        loggingFallbackDependency.setGroupId(Constants.ASK_LOGGING_API_GROUP_ID);
        loggingFallbackDependency.setArtifactId(Constants.ASK_LOGGING_FALLBACK_ARTIFACT_ID);
        loggingFallbackDependency.setVersion(askVersion);
        loggingFallbackDependency.setScope(Scopes.COMPILE);
        return loggingFallbackDependency;
    }

    private AskDependency getProjectProvidedDependencies() throws MojoExecutionException {
        ...
        AskDependency rootDependency = new AskDependency(project.getGroupId(), project.getArtifactId(), null,
        ...
        for (Map.Entry<Artifact, String> entry : dependencyArtifacts.entrySet()) {
            ...
            collectRequest.setRepositories(remoteRepositories);
            AskDependency askDependency;
            try {
                CollectResult collectResult = repositorySystem.collectDependencies(repositorySystemSession, collectRequest);
                DependencyNode node = collectResult.getRoot();

                askDependency = AskDependencyMavenUtil.fromDependencyNode(node, exclusions);

            } catch (Exception e) {
                getLog().error(e);
                throw new MojoExecutionException(e.getMessage());
            }
            rootDependency.addChildDependency(askDependency);
        }
