...
class TestDelegatedMainClass {
  ...
  companion object {
    ...
    fun main(args : Array<String>) {
      ...
      val layout = PatternLayout.newBuilder().withConfiguration(config).withPattern(PatternLayout.SIMPLE_CONVERSION_PATTERN)
      val builder = KBuilder().withFileName(args[0]).withLayout(layout.build()).setConfiguration(config).withName("TEST")
      ...
      ctx.updateLoggers()
      LogManager.getLogger(TestDelegatedMainClass::class.java.name).info("${TestDelegatedMainClass::class.java.name} loaded")
    }
  }
}
